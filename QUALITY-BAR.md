...
- **Screen structure (Palette ADR-0003):** every screen declares an **archetype** and a **shell**, frames with `CanvasPageShell`/`CanvasOverlayShell`/`CanvasFocusedShell`, renders phases through `CanvasStateHost`, and supplies **slot values rather than widgets** — the call site never picks a card-versus-row, and never sizes or clips media. A screen that assembles its own frame, hand-rolls phase branching, omits a phase its archetype requires, or chooses widgets for a slot is a **fail**, not a style preference. Navigation model (drawer vs tabs) is declared by the shell and never switched silently, because it changes the navigation graph.
...
- **Screen structure (Palette ADR-0003):** every screen **declares an archetype** (`list` · `detail` · `form`) and is built from the design system's layout layer — one page frame owning the regions and page inset, one phase host rendering exactly one of loading/empty/error/content. UI phases are a **sum type**, never parallel `loading`/`error` fields, so "loading and error at once" is unrepresentable. A screen that assembles its own frame, hand-rolls phase branching, or omits a phase its archetype requires is a **fail**, not a style preference: component-level conformance alone demonstrably produced nine screens with four loading treatments, four error treatments and three silent empties.
