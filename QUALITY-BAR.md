...
- `minSdk` 26+, `targetSdk` = latest stable; **single-activity** + Navigation Compose.
...
- **DataStore/EncryptedSharedPreferences** for preferences & tokens (Keystore-backed), never plain `SharedPreferences` for secrets.
...
- **Token handling:** access token in **Keystore-backed secure storage**; Bearer interceptor attaches it; **401 → `Authenticator` refresh (rotating)**, single-flight refresh to avoid thundering herd; logout revokes + clears. [EncryptedSharedPreferences](https://developer.android.com/reference/androidx/security/crypto/EncryptedSharedPreferences).
...
- **Gradle Kotlin DSL + version catalog** (`libs.versions.toml`); AGP latest stable; **KSP** for Hilt/Room; Compose BOM (Material3). [Version catalog](https://developer.android.com/build/migrate-to-catalogs).
- **ktlint + detekt** (blocking); `assembleDebug`, `testDebugUnitTest`, `lint`, `ktlintCheck` all green in CI.
- **16 KB page size** support (NDK r27+/AGP) if any native libs.
- **Release:** reproducible/versioned build; **baseline profile** shipped; **App Bundle (AAB)**; proper **minification (R8)**: crash-free ProGuard/R8 rules verified; **signing** kept out of VCS (keystore from env/CI secrets); version catalog bumps via Dependabot/Renovate.
