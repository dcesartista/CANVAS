...
- `minSdk` 34+ (covers ~95%+ of active devices and the post-Android-14 API surface with no legacy workarounds — do not ship lower for new apps), `compileSdk`/`targetSdk` = latest stable (AGP 9 / Compose 1.12+ require `compileSdk 37`); **single-activity** + **Navigation 3** (type-safe, state-driven; Navigation Compose 2.x only as a fallback for deep legacy).
