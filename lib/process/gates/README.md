...
> Universal (process tier) — the protocol is the same everywhere; the exact *commands* are supplied by the Android tier (`lib/android/`).
...
- Run the Android tier's typecheck/build command — must be **clean**.
...
- Run the Android tier's test command — must be **green**; coverage ≥75% on new code (a floor, not a goal).
...
- The auditor loads QUALITY-BAR + the Android impl reference, verifies each *applicable* § against the code with file:line evidence, and emits **pass/partial/fail** per § + an overall score + a must-fix list.
