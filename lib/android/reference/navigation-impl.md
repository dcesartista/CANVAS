# Android — navigation impl (Navigation Compose 2.x, type-safe)

> How navigation Terms ([navigation-theory.md](../../../reference/navigation-theory.md)) are written in native Android with **Navigation Compose 2.x** (`androidx.navigation:navigation-compose`, state-driven, single-activity). Routes are **type-safe**: `@Serializable` route types addressed via `composable<T>` and read with `toRoute<T>()`, replacing string routes and `composable("path/{id}")` entirely.
